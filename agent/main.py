...
class FeatureEncoder:
    ...
    def _get_avail(self, obs, ball_distance):
        ...
        if obs['ball_owned_team'] == 1:
            # opponents owning ball
            avail[LONG_PASS], avail[HIGH_PASS], avail[SHORT_PASS], avail[SHOT], avail[DRIBBLE] = 0, 0, 0, 0, 0
        elif obs['ball_owned_team'] == -1 and ball_distance > 0.03 and obs['game_mode'] == 0:
            # GR ball and far from me
            avail[LONG_PASS], avail[HIGH_PASS], avail[SHORT_PASS], avail[SHOT], avail[DRIBBLE] = 0, 0, 0, 0, 0
        else:
            # self owning ball
            avail[SLIDE] = 0  # dont slide
...
arg_dict = {
    ...
    "trained_model_path": BASEDIR + "/agent/bak/model_133997184.tar",
    "k_epoch": 3,
    "arg_max": True
}
