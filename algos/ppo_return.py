...
class Algo():
    ...
    def train(self, model, data):
        ...
        for mini_batch in data:  # mini_batch: [s, real_a, a, m, r, s_prime, done_mask, prob, need_move]
            ...
            # r_ = r.cpu().numpy()
            # rtgs_lst = []
            # discounted_r = np.array([0])
            # for i in range(len(r_) - 1, -1, -1):
            #     discounted_r = r_[i] + self.gamma * discounted_r * done_mask_[i]
            #     rtgs_lst.append(discounted_r)
            # rtgs_lst.reverse()  # shape: (rollout_len, batch_size, 1)
            # rtgs = torch.tensor(rtgs_lst, dtype=torch.float, device=model.device)
            rtgs = rt
